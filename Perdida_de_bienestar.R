...
########################################################################
#### Proyecto: Perdida de bienestar 2002-2018
#### Documento: Perdida de bienestar 2002-2018
...
# Se eligen los folders donde están las bases
dir_input <- "Bases/"
# Se eligen los folders donde se van a guardar los resultados
dir_output <- "output/"
# Se eligen el nombre del archivo de exportación final
...
## Se generan listas con las claves de gastos para cada categoría usada en el estudio
...
#Generamos los índices de precios
...
## Se genera una variable que identifique al hogar según el decil de ingreso
## en el que se encuentra. Se calculan los deciles usando el hog de expansión
...
## Se colapsan las variables de interés con base en el identificador del hogar
## de manera que quede únicamente una observaciÃ³n por hogar
datos_gasto_inelastico_hogares <- todos_hogares %>% 
  left_join(listas_categorias) %>% 
  # Eliminamos gastos de categorías que no identificamos
  filter(!is.na(tipo)) %>% 
  select(folio, tipo, gasto, año) %>% 
  # Sumamos los gastos por categoría (diferentes claves) y por folio (diferentes hogares)
...
datos_gasto_inelastico_hogares <- datos_gasto_inelastico_hogares  %>% 
  ...
  ## Se genera una variable que agregue el gasto por hogar obtenido a partir de
  ## la base de hogares y la base de personas, sÃ³lo en las categorías de gasto
  ## del análisis
  filter(ingcor>0) %>% 
...
ggplot(datos_gasto_inelastico %>% 
  ...
  theme_minimal()+
  labs(
    title= str_wrap("Distribución de Porcentaje de Gasto InelÃ¡stico sobre Ingreso por Decil", 70),
    subtitle = "La línea roja es una regresión GAM con ajuste suavizado del promedio",
    caption = "Elaboración: Jonathan Guallasamín", 
    x = "Deciles",
...
datos_perdida_bienestar <-   datos_gasto_inelastico %>% 
  ...
  rowwise() %>% 
  #* Se genera una variable que agrega la pérdida de bienestar total considerando
  #* todos los bienes
...
ggplot(temp,
  ...
  theme_minimal()+
  labs(
    title= str_wrap("Porcentaje de Gasto Inelástico con Sobreprecio sobre Ingreso", 80),
    subtitle = "",
    x = "Decil",
    y = "Porcentaje",
    color = "",
